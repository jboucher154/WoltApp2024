...
def test_is_rush_hours_not_rush_hours(not_rush_hours):
    assert not is_rush_hours(not_rush_hours)
...
def test_is_friday_rush_hours_rush_hours(rush_hours):
    assert is_rush_hours(rush_hours)


"""
#  Tests for `calculate_distance_fees' function
- test written with BASE_DISTANCE of 1000 meters before additional fees
"""


@pytest.mark.parametrize("min_fees", [0, 50, 500, 900, 999, 1000])
...
@pytest.mark.parametrize("larger_fees", [1001, 1300, 1500, 1501, 2000, 2499])
...
def test_calculate_distance_fees_one_extra_fee():
    distance = 1001
    assert (
        calculate_distance_fees(distance)
        == delivery_fee_macros.BASE_DISTANCE_FEE
        + delivery_fee_macros.ADDITIONAL_DISTANCE_FEE
    )


def test_calculate_distance_fees_two_extra_fees():
    distance = 1501
    assert calculate_distance_fees(
        distance
    ) == delivery_fee_macros.BASE_DISTANCE_FEE + (
        delivery_fee_macros.ADDITIONAL_DISTANCE_FEE * 2
    )


def test_calculate_distance_fees_4500():
    distance = 4500
    expected = delivery_fee_macros.BASE_DISTANCE_FEE + (
        delivery_fee_macros.ADDITIONAL_DISTANCE_FEE * 7
    )
...
def test_calculate_distance_fees_4501():
    distance = 4501
    expected = delivery_fee_macros.BASE_DISTANCE_FEE + (
        delivery_fee_macros.ADDITIONAL_DISTANCE_FEE * 8
    )
    fees = calculate_distance_fees(distance)
    assert fees == expected
...
@pytest.mark.parametrize("normal_orders", [1300, 2000, 1000, 1001])
def test_check_for_small_order_fee_not_applied(normal_orders):
    assert check_for_small_order_fee(normal_orders) == 0


@pytest.mark.parametrize("small_orders", [700, 999, 1, 300, 75])
...
- test cases written with large order surcharges starting at 5 items and 
bulk order starting at 13
"""


@pytest.mark.parametrize("no_charges", [1, 2, 3, 4])
def test_calculate_item_count_surcharges_none(no_charges):
    assert calculate_item_count_surcharges(no_charges) == 0


@pytest.mark.parametrize(
    "large_charges, expected_number_of_fees",
    [(5, 1), (6, 2), (7, 3), (8, 4), (9, 5), (10, 6), (11, 7)],
)
def test_calculate_item_count_surcharges_large(large_charges, expected_number_of_fees):
    assert (
        calculate_item_count_surcharges(large_charges)
        == expected_number_of_fees * delivery_fee_macros.LARGE_ORDER_ITEM_FEE
    )


@pytest.mark.parametrize(
    "mixed_charges, expected_num_fees_to_apply",
    [(3, 0), (6, 2), (1, 0), (12, 8), (15, 11), (13, 9)],
)
def test_calculate_item_count_surcharges_none(
    mixed_charges, expected_num_fees_to_apply
):
